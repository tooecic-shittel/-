...
from app.services import digital_human, llm, material, subtitle, video, voice, upload_post
...
def generate_digital_human_clip(task_id, params, video_script, audio_file):
    if not getattr(params, "digital_human_enabled", False):
        return ""

    logger.info("\n\n## generating digital human talking head")
    digital_human_clip = digital_human.generate(
        task_id=task_id,
        photo_file=getattr(params, "digital_human_photo_file", ""),
        audio_file=audio_file,
        script_text=video_script,
    )
    if not digital_human_clip:
        logger.warning("digital human clip was not generated; fallback to normal video flow")
        return ""
    return digital_human_clip


def get_video_materials(task_id, params, video_terms, audio_duration, allow_empty=False):
    if params.video_source == "local":
        ...
        if not materials:
            if allow_empty:
                logger.warning("no valid local materials found, continue with generated spokesperson clip only")
                return []
            sm.state.update_task(task_id, state=const.TASK_STATE_FAILED)
            logger.error(
        ...
        if not downloaded_videos:
            if allow_empty:
                logger.warning("failed to download platform materials, continue with generated spokesperson clip only")
                return []
            sm.state.update_task(task_id, state=const.TASK_STATE_FAILED)
            logger.error(
...
def start(task_id, params: VideoParams, stop_at: str = "video"):
    ...
    # Optional: generate a talking-head clip from the merchant photo and narration.
    # It is treated as a normal video material afterwards, so the stable composition
    # path remains unchanged.
    digital_human_clip = generate_digital_human_clip(
        task_id=task_id,
        params=params,
        video_script=video_script,
        audio_file=audio_file,
    )
    if digital_human_clip:
    ...
    downloaded_videos = get_video_materials(
        ...
        video_terms,
        audio_duration,
        allow_empty=bool(digital_human_clip),
    )
    if not downloaded_videos and not digital_human_clip:
        sm.state.update_task(task_id, state=const.TASK_STATE_FAILED)
        return
    ...
    if digital_human_clip:
        downloaded_videos = [digital_human_clip] + downloaded_videos
    ...
    kwargs = {
        ...
        "subtitle_path": subtitle_path,
        "materials": downloaded_videos,
        "digital_human_video": digital_human_clip or None,
