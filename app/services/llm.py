...
# =============================================================================
# E-commerce viral reference assistant
#
# 这组函数只负责把“参考爆款 + 商品信息”整理成官方主链路已经支持的
# video_script / video_terms / video_script_prompt。它不接管视频合成、配音或素材下载。
# =============================================================================
...
def _metadata_summary(metadata: dict[str, Any]) -> str:
    ...
    duration_text = f"{int(duration)} 秒" if isinstance(duration, (int, float)) else ""
    ...
    return "；".join(
        ...
        for part in [
            ...
            f"时长：{duration_text}" if duration_text else "",
...
def _fallback_style_prompt(metadata: dict[str, Any]) -> str:
    reference = _metadata_summary(metadata) or "该链接未解析到完整元数据。"
    return f"""参考爆款信息：{reference}

请按竖屏 9:16 电商短视频处理：开头 3 秒必须有强钩子，直接给出问题、反差或结果；中段用 3-5 个短镜头推进信息，每个镜头只表达一个重点；结尾给出自然行动引导。整体节奏紧凑、口语化、真实商家感强，字幕高对比大字突出关键词。只学习参考视频的结构、节奏、情绪和镜头语言，不复制原视频人物、品牌、商标、水印、具体画面或受版权保护的表达。"""


def generate_style_prompt_from_video(
    metadata: dict[str, Any],
    use_llm: bool = True,
) -> tuple[str, bool]:
    fallback = _fallback_style_prompt(metadata)
    if not use_llm:
        return fallback, False

    prompt = f"""
# Role: Short Video Style Prompt Generator

## Goal
Analyze public metadata from a Douyin/TikTok video link and generate a reusable Chinese style prompt for an e-commerce short-video workflow.

## Constraints
1. Reply in Chinese.
2. Do not claim you watched the actual frames unless frame data is provided.
3. Infer style only from title, description, author, tags, duration, and available metadata.
4. Output one practical prompt that can guide script adaptation and stock/local material selection.
5. Include opening hook, narrative structure, pacing, camera language, subtitle style, music/emotion, and what must not be copied.
6. No markdown title.
...
""".strip()
    ...
    if not response or "Error: " in response:
        logger.warning(f"failed to generate Douyin style prompt with LLM: {response}")
        return fallback, False
    return response.strip(), True


def _fallback_ecommerce_script(
    product_name: str,
    selling_points: str,
    speaker_role: str,
    style_prompt: str,
) -> str:
    product_name = (product_name or "这款产品").strip()
    selling_points = (selling_points or "品质稳定、真实好用、适合日常场景").strip()
    speaker_role = (speaker_role or "老板").strip()
    style_hint = "整体节奏紧凑，表达自然。"
    if style_prompt:
        style_hint = "整体参考已提取的爆款节奏，保留强钩子和口语化表达。"

    return (
        f"先别急着划走，我是{speaker_role}。很多人选{product_name}，最怕买回去不适合。"
        f"我们主打{selling_points}。想找一个省心、真实、日常用得上的选择，"
        f"可以先来了解一下。{style_hint}"
    )


def generate_ecommerce_spokesperson_script(
    product_name: str,
    selling_points: str,
    speaker_role: str = "老板",
    style_prompt: str = "",
    ...
    fallback = _fallback_ecommerce_script(
    ...
    prompt = f"""
# Role: E-commerce Short Video Spokesperson Script Writer

## Goal
Generate a Chinese short-video spokesperson script for an e-commerce merchant. It should sound like a real shop owner, founder, or staff member speaking on camera.

## Constraints
1. Reply in Chinese.
2. Return only the spoken script, no title, no markdown, no explanation.
3. Start with a strong hook in the first sentence.
4. Use natural spoken language, not a slogan pile.
5. Keep it suitable for vertical short video.
6. Include trust, product value, realistic use scenario, and a gentle call to action.
7. Avoid unverifiable claims such as "best", "number one", "guaranteed", medical claims, or impossible outcomes.
8. Keep the final script short: about 80-140 Chinese characters, 4-6 short spoken sentences, roughly 20-30 seconds.
...
""".strip()
    ...
    response = _generate_response(prompt)
    if not response or "Error: " in response:
        logger.warning(f"failed to generate ecommerce spokesperson script: {response}")
        return fallback, False
    return response.strip().replace("*", "").replace("#", ""), True


def _fallback_adapt_reference_script(
    reference_text: str,
    style_prompt: str,
    product_name: str,
    selling_points: str,
    ...
    product_name = (product_name or "这款产品").strip()
    selling_points = (selling_points or "真实好用、细节扎实、适合日常使用").strip()
    speaker_role = (speaker_role or "老板").strip()
    reference_hint = "参考爆款的开场节奏和口语表达"
    if reference_text:
        reference_hint = "参考爆款的钩子、转折和表达节奏"
    if style_prompt:
        reference_hint = "参考已提取的视频风格、镜头节奏和口语化表达"
    if current_script:
        reference_hint += "，同时保留当前文案里已经明确的商品信息"

    return (
        f"先别急着划走，我是{speaker_role}。很多人买{product_name}，最怕到手不好用。"
        f"我们这款重点是{selling_points}。整体按{reference_hint}来讲，"
        f"但内容全部换成我们自己的商品。想体验的话，可以先了解一下。"
    )


def adapt_reference_video_script(
    reference_text: str,
    style_prompt: str,
    product_name: str,
    selling_points: str,
    ...
    fallback = _fallback_adapt_reference_script(
    ...
    prompt = f"""
# Role: E-commerce Short Video Adaptation Writer

## Goal
Adapt the reference short-video copy/style into a new Chinese e-commerce spokesperson script for the user's own product or store.

## Constraints
1. Reply in Chinese.
2. Return only the final spoken script. No title, no markdown, no explanation.
3. Do not copy the reference script sentence by sentence. Keep only reusable structure, rhythm, hook style, emotional angle, and pacing.
4. Replace all concrete product, brand, person, place, price, promise, and offer details from the reference with the user's own product information.
5. Make it natural for a real shop owner, founder, or staff member to say on camera.
6. Avoid unverifiable claims such as "best", "number one", guaranteed results, medical claims, or exaggerated promises.
7. If user product information is limited, write a safe generic version and avoid inventing hard facts.
8. Keep the final script short: about 80-140 Chinese characters, 4-6 short spoken sentences, roughly 20-30 seconds.
...
## Existing Draft Script
{current_script}

## Reference Video Copy / Metadata
{reference_text}

## Extracted Reference Style Prompt
{style_prompt}
""".strip()

    response = _generate_response(prompt)
    if not response or "Error: " in response:
        logger.warning(f"failed to adapt reference video script: {response}")
        return fallback, False
    return response.strip().replace("*", "").replace("#", ""), True
