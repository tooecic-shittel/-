...
class TestEcommerceAssistant(unittest.TestCase):
    def test_style_prompt_fallback_uses_metadata_without_claiming_frames(self):
        prompt, used_llm = llm.generate_style_prompt_from_video(
            {
                "title": "火锅店爆款探店",
                "description": "开场直接展示锅底和招牌菜",
                "duration": 18,
                "tags": ["火锅", "探店"],
            },
            use_llm=False,
        ...
        self.assertIn("火锅店爆款探店", prompt)
        self.assertIn("只学习参考视频的结构", prompt)
        self.assertNotIn("我看到了", prompt)

    def test_ecommerce_script_fallback_mentions_product_and_role(self):
        script, used_llm = llm.generate_ecommerce_spokesperson_script(
            product_name="小龙坎火锅",
            selling_points="食材新鲜，锅底现炒",
            speaker_role="老板",
            use_llm=False,
        )

        self.assertFalse(used_llm)
        self.assertIn("小龙坎火锅", script)
        self.assertIn("老板", script)
        self.assertIn("食材新鲜", script)
